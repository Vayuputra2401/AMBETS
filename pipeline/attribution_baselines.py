...
-------
  gradcam   : Grad-CAM at the post-CCR bottleneck block (decoder.enc3, 16³, 192-dim)
              ...
              Per concept k: backprop mean(seg_logits[:,k]) → grads at enc3 →
              channel-weighted, ReLU, trilinear-upsampled to 128³.
  gradient  : Vanilla gradient saliency — |∂ mean(seg_logits[:,k]) / ∂ input|,
              max over the 4 MRI channels → 128³ map.

Both explanation maps are scored with the IDENTICAL deletion/insertion protocol
used for CCR (evaluation.deletion_auc_from_map / insertion_auc_from_map).

The expected story: post-hoc deletion AUC lands ~0.55–0.68 (structural ceiling,
cf. arXiv:2603.22624), well below CCR routing's ~0.84–0.89.

Usage
-----
    # Default: first 40 test cases, gradcam + gradient (deletion is ~expensive)
    python pipeline/attribution_baselines.py \
        --checkpoint /path/epoch_0080.pth --env gcp --split test --n_cases 40

    # Full test set, single method:
    python pipeline/attribution_baselines.py \
        --checkpoint ... --env gcp --split test --n_cases 0 --methods gradcam
...
--------
    ...
    --split        val | test
    --methods      comma list: gradcam,gradient (default both)
...
def main() -> None:
    ...
    parser.add_argument("--methods",    type=str, default="gradcam,gradient")
    ...
    for batch in tqdm(loader, total=n_cases, desc="  baselines"):
        ...
        for method in methods:
            maps = explanation_maps(method, model, gradcam, image, concept_indices, target_shape)
            for k in concept_indices:
