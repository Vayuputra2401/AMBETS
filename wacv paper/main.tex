...
loss), while the routing aligns robustly even without the warm-up phase. Dropped \emph{unchanged}
into a SegResNet CNN (CCR-SegResNet), the same module reproduces every result from scratch
($\casfg$ up to $0.96$, concept-discriminability $0.88$), so the principle is backbone-agnostic,
not architecture-specific. CCR is the first method to make the routing decision itself the
faithful, clinically-labeled explanation for 3D volumetric dense prediction.
...
\begin{tabular}{lccc}
\toprule
Instantiation & Dice WT/TC/ET & $\casfg$ N/E/T & Concept-discrim.\ N/E/T \\
\midrule
CCR-Net (Swin, pretrained)   & $0.915/0.845/0.837$ & $0.69/0.94/0.93$ & $0.85/0.89/0.87$ \\
CCR-SegResNet (CNN, scratch) & $0.927/0.883/0.869$ & $0.80/0.96/0.95$ & $0.85/0.91/0.89$ \\
\bottomrule
\end{tabular}
\end{table}
...
metric they optimize but cannot tell the concepts apart. The principle is \emph{backbone-agnostic}:
the same module, dropped unchanged into a SegResNet CNN (CCR-SegResNet), reproduces the concept
alignment, discriminability, and segmentation from scratch. CCR is the first method to deliver
