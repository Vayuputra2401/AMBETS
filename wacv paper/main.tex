...
\title{Clinical Concept Routing: Faithful Intrinsic\\ Explanation for 3D Medical Image Segmentation}
...
The dominant remedy---post-hoc attribution (Grad-CAM, gradients, SHAP)---computes the
explanation \emph{after} the prediction, from byproducts of computation, and a recent
benchmark shows it fails faithfulness tests on dense prediction, with deletion AUC stuck
near the random baseline. We propose \textbf{Clinical Concept Routing (CCR)}, a different
...
Alignment Score (CAS). On the BraTS 2024 glioma benchmark, CCR-Net attains $\casfg$ up to
$0.94$ and, on an identical deletion/insertion protocol, a deletion AUC of $0.82$--$0.86$
for edema and enhancing tumor, versus $\le 0.66$ for gradient saliency and $\le 0.40$ for
Grad-CAM. Ablations confirm that concept alignment is \emph{learned} (it collapses without
the alignment loss) and that a warm-up phase is necessary to prevent expert collapse. CCR
is the first method to make the routing decision itself the faithful, clinically-labeled
explanation for 3D volumetric dense prediction.
...
attention weights), not from the computation itself. It approximates what happened; it is
not what happened. The empirical ceiling is real: measured by the
deletion/insertion game~\cite{petsiuk2018rise}, post-hoc deletion AUC on segmentation
sits at $0.55$--$0.68$, barely above the random baseline of $0.50$, and no amount of
algorithmic refinement closes the gap because it is structural~\cite{rudin2019stop}.
...
(Sec.~\ref{sec:theory}).
\item \textbf{A measured faithfulness gap.} On an identical deletion/insertion protocol,
CCR routing attains deletion AUC $0.82$--$0.86$ (edema, enhancing tumor) against $\le0.66$
for the strongest post-hoc baseline and $\le0.40$ for Grad-CAM (Sec.~\ref{sec:experiments}).
...
prediction and treat a byproduct of computation as the decision. On dense prediction their
faithfulness, measured by deletion/insertion~\cite{petsiuk2018rise}, is near the random
floor. CCR does not improve post-hoc attribution; it removes the structural reason for its
failure by making the explanation intrinsic~\cite{rudin2019stop}.
...
$\mathrm{CAS}\cdot(1-\mathrm{DD})=0.19,\,0.44,\,0.56$ for NCR, edema, and ET; the deletion AUC
we measure (Table~\ref{tab:faith}) exceeds each, so the bound is satisfied empirically. A full
retrofit study on a frozen third-party backbone is left to future work.
...
one-vs-rest AUROC per concept; and deletion/insertion AUC for faithfulness.
...
\begin{table}[t]
\centering
\caption{Segmentation, alignment, and per-concept AUROC on the BraTS test split
($n{=}168$). $\casfg$ is the foreground concept-alignment score.}
\label{tab:main}
\begin{tabular}{lccc}
\toprule
& WT / NCR & TC / Edema & ET \\
\midrule
Dice                       & $0.915$ & $0.845$ & $0.836$ \\
$\casfg$ (NCR/Edema/ET)    & $0.682$ & $0.935$ & $0.931$ \\
AUROC (NCR/Edema/ET)       & $0.709$ & $0.833$ & $0.774$ \\
\bottomrule
\end{tabular}
\end{table}

\subsection{Faithfulness vs.\ post-hoc attribution}\label{sec:faith}
We compare CCR routing against two post-hoc baselines computed on the same trained
CCR-Net: Grad-CAM at the post-CCR bottleneck block (the same location the routing lives,
for a fair contrast) and vanilla gradient saliency. All three are scored with the identical
protocol of Eq.~\ref{eq:auc}. Figure~\ref{fig:faith} and Table~\ref{tab:faith} report the
result on a matched set of cases.

On edema and enhancing tumor---the concepts with enough spatial extent to measure reliably
($n{=}40$)---CCR is decisively more faithful on \emph{both} deletion and insertion:
deletion AUC $0.82$--$0.86$ versus $0.46$--$0.66$ for the strongest baseline (gradient) and
$0.38$--$0.40$ for Grad-CAM; a gap of $+0.28$ over the best baseline and $+0.45$ over
Grad-CAM, with insertion showing the same ordering. This is the central empirical result:
removing the voxels CCR calls important collapses the prediction, whereas removing the
voxels Grad-CAM calls important barely moves it---precisely the faithfulness that post-hoc
methods lack. Necrotic core is measured on fewer cases ($n{=}16$, since many cases contain
no NCR); CCR leads on insertion, while the small-sample deletion comparison is inconclusive
and we do not claim NCR superiority. Gradient saliency consistently outperforms Grad-CAM, so
CCR's margin holds against the stronger baseline.

\begin{figure}[t]
\centering
\includegraphics[width=\textwidth]{figures/fig3_faithfulness.png}
\caption{Faithfulness on a matched set (deletion left, insertion right; higher is more
faithful, Eq.~\ref{eq:auc}). CCR (green) dominates gradient saliency and Grad-CAM on edema
and ET; the dotted line marks the $0.5$ reference. NCR is measured on $n{=}16$ cases.}
\label{fig:faith}
\end{figure}

\begin{table}[t]
\centering
\caption{Deletion / Insertion AUC (higher = more faithful), matched cases, identical
protocol. Best per row in \textbf{bold}. $\dagger$NCR is measured on $n{=}16$ (high
variance).}
\label{tab:faith}
\begin{tabular}{llccc}
\toprule
...
\midrule
\multirow{3}{*}{Deletion}
 & Edema & $\mathbf{0.857}$ & $0.658$ & $0.397$ \\
 & ET    & $\mathbf{0.821}$ & $0.462$ & $0.376$ \\
 & NCR$^\dagger$ & $0.667$ & $0.787$ & $0.405$ \\
\midrule
\multirow{3}{*}{Insertion}
 & Edema & $\mathbf{0.824}$ & $0.469$ & $0.374$ \\
 & ET    & $\mathbf{0.746}$ & $0.389$ & $0.326$ \\
 & NCR$^\dagger$ & $\mathbf{0.532}$ & $0.461$ & $0.302$ \\
\bottomrule
\end{tabular}
\end{table}
...
quality equal to the Concept Alignment Score. On an identical causal protocol the routing
is far more faithful than Grad-CAM or gradient saliency on the concepts that admit reliable
measurement. CCR is the first method to deliver intrinsic, concept-labeled, faithful
explanation for 3D volumetric dense prediction; extending the principle to arbitrary
backbones (Proposition~\ref{prop:1b}) and to additional anatomies is promising future work.
