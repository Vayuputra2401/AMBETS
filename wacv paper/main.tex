...
\subsection{CCR-Retrofit: a proposed test of Conjecture~\ref{prop:1b}}\label{sec:retrofit}
\emph{We have not run this experiment; we describe it because it is the natural test of
Conjecture~\ref{prop:1b} and we want the prediction on record before the fact.}
Conjecture~\ref{prop:1b} concerns a \emph{frozen}, independently-trained backbone in which only
the router and experts are trained---a setting where the decoder cannot adapt to the routing at
all. Testing it would mean freezing a pretrained Swin-UNETR, inserting the CCR module at the
stage-2 bottleneck, training only the CCR parameters under
$\mathcal{L}_{\text{align}}$, and checking the measured deletion AUC against
$\mathrm{CAS}\cdot(1-\mathrm{DD})$. The prediction is a substantially larger DD than end-to-end
CCR-Net---a frozen decoder has no incentive to track the routing---with the inequality still
holding. A failure would be informative in the other direction: it would show the two error
sources do not compose as the conjecture assumes.
...
regime Conjecture~\ref{prop:1b} describes, and the conjectured inequality holds on every concept
we measured (Fig.~\ref{fig:bound})---evidence for it, though not a proof. Testing it on a frozen
third-party backbone (Sec.~\ref{sec:retrofit}), extending to additional anatomies, and a
controlled plain-backbone accuracy comparison (Sec.~\ref{sec:limitations}) are the natural next
steps.
