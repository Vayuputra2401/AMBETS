...
phase. Ablations run to $40$ epochs (their effect is evident well before convergence).
\todo{A4 numbers pending.}}
\label{tab:abl}
\begin{tabular}{lccc}
\toprule
& Full CCR-Net & A1: no $\mathcal{L}_{\text{align}}$ & A4: no warm-up \\
\midrule
$\casfg$ (Edema)            & $0.935$ & $0.322$  & \todo{$\sim$} \\
$\casfg$ (ET)               & $0.930$ & $0.094$  & \todo{$\sim$} \\
$\casfg$ (NCR)              & $0.688$ & $-0.011$ & \todo{$\sim$} \\
Dice (WT)                   & $0.915$ & $0.905$  & \todo{$\sim$} \\
Max expert utilization (\%) & $28$    & $32$     & \todo{$\sim$} \\
\bottomrule
\end{tabular}
\end{table}
...
by-product of segmentation---exactly the temporal signal in Fig.~\ref{fig:curve}. Experts stay
balanced (max utilization $32\%$), so A1 yields misalignment \emph{without} collapse. A4 (no
warm-up) is expected to drive utilization to a single dominant expert, degrading both CAS and
Dice; \todo{A4 pending.}
