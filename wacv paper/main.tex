...
$\approx\!0.5$ (chance throughout). The margin is significant against every baseline (paired
Wilcoxon $p<0.001$; pooled CCR $0.88$ $[0.87,0.89]$ vs.\ occlusion $0.73$ $[0.71,0.74]$---the
strongest---a gap of $+0.15$ with non-overlapping bootstrap CIs). The very methods that win
deletion cannot tell the concepts apart: their per-class maps highlight the same high-signal
tumor structure regardless of $k$. CCR
...
methods dominate on the left axis, CCR on the right. Figure~\ref{fig:qualmaps} shows the same
effect qualitatively---CCR's three per-concept maps differ and fall inside the ground-truth
boundaries, whereas the sensitivity methods produce nearly the \emph{same} map for every concept.
